...
from git_commit_analyzer.checkers import SubjectMatchesRegexChecker, SubjectMaxLengthChecker
from git_commit_analyzer.filters import DirectoryModifiedFilter
...
class TestRule:
    def test_filtered_out_commit_returns_none(self):
        rule = Rule(
            name="src-rule",
            filters=[DirectoryModifiedFilter(directories=["src"])],
            checkers=[SubjectMaxLengthChecker(max_length=10)],
        )
    ...
    def test_multiple_checkers_all_failures_collected(self):
        rule = Rule(
            ...
            checkers=[
                ...
                SubjectMaxLengthChecker(max_length=5),
            ],
        )
    ...
    def test_no_filters_applies_to_all_commits(self):
        rule = Rule(name="all", checkers=[SubjectMaxLengthChecker(max_length=72)])
...
class TestRuleset:
    def test_returns_only_failing_results(self):
        ruleset = Ruleset(rules=[
            Rule(name="conventional", checkers=[SubjectMatchesRegexChecker(pattern=r"^feat: ")]),
        ])
        commits = [
    ...
    def test_multiple_rules_each_can_fail_independently(self):
        ruleset = Ruleset(rules=[
            Rule(name="rule-a", checkers=[SubjectMatchesRegexChecker(pattern=r"^feat: ")]),
            Rule(name="rule-b", checkers=[SubjectMaxLengthChecker(max_length=5)]),
        ])
